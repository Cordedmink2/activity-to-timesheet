...
- `python scripts/category_rules.py --status` — **before the timeline below**, because the timeline labels every span with these rules and a run that reads them stale mislabels the whole day. Two local file reads and nothing over the wire, so it costs a run nothing:
  ...
  - **`STALE …`** — a client's signals have moved since the rules were written, or this plugin has never written them. Rebuild before reading the timeline: compose one candidate per title-visible signal per client out of the `## Active client projects` section you have just read, and run `python scripts/category_rules.py --candidates -` with them (the `setup` skill's category step has the shape of the candidates and the signal types). Say in one line what changed — "your rules now include Beta" — and carry on. This needs no approval of its own: the user's signals are what they already declared, and the rules are a derived copy of them.
