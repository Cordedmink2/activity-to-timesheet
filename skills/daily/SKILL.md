...
**Timezone:** AW stores UTC; the scripts convert using the configured `TIMESHEET_TIMEZONE` — or, where none is configured, this machine's own zone — applying it at each instant rather than once for the day, so a daylight-saving change needs nothing from you — including on the day itself, which is 23 or 25 hours long and is read at that length. On the day the clocks go back, the second pass over the repeated hour is suffixed `*` (`02:30:00*`); hand it back to `--window` / `--cover` as part of the time, and see `references/output-format.md` before it reaches Harvest. **There is no assumed zone and no default offset.** A zone read from the machine is labelled `derived from this machine` in every skeleton header and JSON `zone` field; **carry that label into the timesheet's Notes and the Step 8 confirmation**, so a wrong one is seen before it is billed from — a configured zone needs no note. A run that can read neither a configured zone nor the machine's stops and says so, because a guessed offset dates the day wrong without failing. `--utc-offset <hours>` still overrides both for a single run (a day spent in another zone). If a run reports it cannot *load* the zone, the machine is missing the zone database: `py -m pip install tzdata`.
...
- `python scripts/category_rules.py --status` — **before the timeline below**, because the timeline labels every span with these rules and a run that reads them stale mislabels the whole day. Two local file reads and nothing over the wire, so it costs a run nothing:
  ...
  - **`STALE …`** — a client's signals have moved since the rules were written, or this plugin has never written them. Rebuild before reading the timeline: compose one candidate per title-visible signal per client out of the `## Active client projects` section you have just read, and run `python scripts/category_rules.py --candidates -` with them (the `setup` skill's category step has the shape of the candidates and the signal types). **Pass every client in the file, not only the one that changed** — the managed set is regenerated whole, so a client left out of the candidates has its rule removed, which the run reports as `DROPPED`. That is what makes a client you retired stop labelling spans, and what would silently retire one you merely forgot to include. Say in one line what changed — "your rules now include Beta" — and carry on. This needs no approval of its own: the user's signals are what they already declared, and the rules are a derived copy of them.
  - **A rebuild the gate refuses does not stop the run.** Report the refusal, carry on with the rules as they stand, and raise it at Step 11 — the category is a first-pass client signal that Steps 4 and 5 check anyway, so a day drafted against yesterday's rules is a day with more 🔸 in it, not a wrong one. (The calendar is the opposite case for the opposite reason: a source the user turned on and did not get loses meetings silently.)
...
- **`work_end` = the script's `work_end`** (last `not-afk` moment) — a *ceiling*, not a magnet. The day never ends at the last window event: a terminal/IDE/browser left in focus reads as "still working" for minutes or hours after the user walked away (the script's `window_watcher_tail` flag marks this). Never extend the last block to the screenshot window (08:30–20:00) or the work-hours rendering window (06:00–20:00) — those are capture/display bounds, not activity signals. And when `work_end` itself is a momentary blip (a sub-2-min `not-afk` flicker after ≥10 min idle — the script flags this too), end the final block at its last *substantive* active span, not at the blip. (A calendar block may end after it: a corroborated event whose `block` span runs later, below, or an uncorroborated one the user accepts at review — Step 6. Both are declared.)
- **A block runs from an active-span start to the next break start (or `work_end`).** It never crosses a break, and it never stops early because short AFKs cluster near its tail. (A calendar block may cross a break: a corroborated event, below, or one the user accepts at review — Step 6. Both are declared.)
- **Short AFK folds in.** AFK events `< 1050s` are not boundaries — bathroom/coffee stays inside the block, duration not subtracted.

**Validate every block you derived from the skeleton with its `active_ratio`** (get it per block via `afk_blocks.py <date> --window HH:MM-HH:MM`; the default output covers only whole spans). A `.context.md` billing convention can override the ratio verdict for a specific item (e.g. a standup the user joins by phone bills despite a low ratio) — and a block the *user* dictated is not ratio-validated at all: a calendar question they accepted at review (Step 6) has their answer as its evidence, which is that same convention given live instead of in advance. The bands below would refuse it, since a meeting the machine never saw scores 0.0:
...
**An uncorroborated event is never drafted as a block.** No meeting window intersected it, and the calendar's word alone does not bill: a meeting skipped, declined in practice, or taken from the car in front of a client's office all look identical from here, and only the user can tell them apart. Leave it out of the table and out of the coverage check. It goes under the table at Step 6 as a **calendar question** instead, and becomes a block only on the user's answer.
...
**Calendar questions** — every uncorroborated event still unanswered, under its own heading below the table, one line each:

```
Calendar questions (not drafted, not billed unless you say so):
- 07:30–08:30 · Northwind site visit · Ana Client — no meeting window. Bill it?
```

None is a block (Step 3), so none is in the table, in the block counts, or in anything passed to guard 3. They are not 🔸 either — a 🔸 is a drafted block.

- **Batch-accept covers the drafted blocks and nothing else** — each calendar question takes its own answer, a word each.
- **A yes makes it a block over the event's span**, classified like any other (Steps 4 and 5) and declared under the table naming the event and the user's answer. It may sit anywhere they say they were: outside the active spans (a client site before the laptop opened), across a break (a client lunch), or past `work_end`. The guards do not touch it (guard 1). It **replaces what it covers rather than doubling it**, exactly as a corroborated block does (Step 3): a yes to a 15:00–15:30 call inside a drafted 13:30–17:00 stretch splits that stretch into 13:30–15:00, the call, and 15:30–17:00 — never a fourth entry over the top. `--cover` reads both drafts as covered, so this rule is the only check on it. The 0.25 hr minimum still holds: an event shorter than that folds into a same-client neighbour or goes back to the user, never posts as a sliver. Then it is an entry like any other: Step 8 lists it and passes it to `--cover`.
- **A no, or no answer, leaves it unbilled** — out of the table, out of `--cover`, out of Step 9. Ask once.
...
> "I've drafted N blocks for <date> (AFK watcher: work ended HH:MM, breaks at …; blocks cover all active spans). M are high confidence, K are 🔸, and Q calendar question(s) are pending under the table — batch-accept doesn't cover those. Walk through the 🔸 ones, or batch-accept and edit after?"

Leave the calendar clause out when Q is zero.
...
- [ ] No block past `work_end`; no block crossing a script break — except a calendar block, which may do both and is declared under the table: a corroborated one over its `block` span (Step 6 guards 1 and 2), or one the user accepted at review, which the guards leave alone as their instruction (Step 6 guard 1)
...
- [ ] All 🔸 blocks resolved with the user, and every calendar question put to them — an accepted one is an entry on this list; a no or a silence is not one and is not posted
...
>
> Times in Pacific/Auckland, derived from this machine; set TIMESHEET_TIMEZONE to pin it.
>
> Proceed? (yes / no / edit block <n>)"

The zone line is there only when the skeleton's header read `derived from this machine` — a configured zone is the user's own choice and is not announced. It is the last moment the user sees the zone before anything reaches the provider, and a wrong one dates every entry above it.
...
Summarise: total hours posted, blocks deferred/skipped, and whether more days are outstanding — but **don't name the next date here**; Step 12 does that after the reset ask. Save the entry ids to `Timesheets/<date>_harvest_responses.json`: `{"entries": [{"id": 2933345845, "project_code": "NWC-001", "hours": 0.25, "notes": "..."}], ...}`. **Record every calendar question the user answered there too — the yeses and the noes, with the answer** — an accepted event's block has no activity evidence behind it, and this file is where Step 1's already-covered branch looks for a ruling before proposing to change a day. A recorded no is a ruling like any other: it binds, and that date does not ask again.
...
**A yes to a diff that adds or changes a client's signals rebuilds the category rules in the same breath** — write the file, then run `scripts/category_rules.py --candidates -` with the new signal set, and report what it wrote in a line. Don't ask a second time: the rules are a derived copy of the signals they just approved, and a second approval for the derived thing is a question with no other answer. It is also what keeps the promise the staleness check at Step 2 makes — a signal added today matches tomorrow, without anybody remembering to re-run setup. A refusal from the gate is reported here rather than at Step 2, since this is where it happened.
...
- `docs/skills/daily/decision-log.md` — the evidence behind the rules, and where a new finding about the skill goes rather than into `SKILL.md`.
...
- `scripts/category_rules.py` — the activity source's **category rules**: gate, order, back up, write, verify (#69). It takes *candidate rules* — `{client, signal, pattern}` objects, from a file or from stdin on `--candidates` — rather than reading `.context.md` itself, because composing a pattern out of a file that holds prose and hints is judgement and judging what was composed is not. The gate refuses a pattern that does not compile, one that matches none of the sampled window titles, and one that matches more than `--max-share` of them (0.35 by default); a refusal anywhere writes nothing at all, since the rule set is written whole. A **scoped** signal is judged against the titles of the application it is scoped to rather than the whole sample — the ceiling was calibrated on a rule matching 256 of 552 *browser* titles, and diluting that by a day's Explorer and editor windows would let the very rule it was set from through. What it writes is ordered by signal type, most specific first, with the fallback signals — a profile tag, a browser profile's name — written **below even the rules this plugin did not author**, so the more specific evidence wins a span two rules both match whoever wrote them; a signal type that never reaches a window title, a repo path, is skipped with the reason said out loud rather than refused. The previous rule set is copied into `<workspace>/.mcp/` before the first write, which is the recovery path and is what makes the write safe without showing anyone a diff. The managed set is regenerated whole — a client no longer among the candidates has its rule removed and the run says `DROPPED` — while rules this plugin did not author are kept exactly as they were, `id` and all, unless a candidate names one in `adopts` to take it over. Then it reads the rules back and re-matches each, because a write that did not land is the difference between a configured install and one that looks configured — and it records what it wrote only once that verify has passed, so a write that did not land leaves the rules `STALE` and the next run rebuilds them rather than believing yesterday's stamp. `--days` sets how much of the recent past the sample covers (7), and `--workspace` overrides where the backup and the stamp go. `--inspect` reports the rules the activity source holds now — `[managed]` for the ones written from this plugin's own stamp, `[unmanaged]` for the user's own, and `EDITED` for a managed one whose pattern is no longer what was written, which is where a rule damaged in the settings dialog surfaces since the staleness check reads no rules — each with the share of the sample it matches and an example of what it caught, which is the read an adoption of pre-existing rules is made from; and `--status` says whether the rules are still current for `Timesheets/.context.md`, reading two local files and nothing over the wire. Nothing reads those rules back as authority: `.context.md` is the source of truth and they are a derived copy
...
- `scripts/timezone.py` — the zone a day is read in (`TIMESHEET_TIMEZONE`, or the machine's own zone where none is configured, labelled as derived) and the clock arithmetic that follows from it: the day's UTC bounds, the local-clock rendering and the range parsing behind the window and coverage flags above, and where on a date the clocks change. Not a script, and not either half's: the two reading scripts above and the two writers below all import it, and its conversions apply the offset in force at each instant rather than one read once for the day, which is what makes the day the clocks change 23 or 25 hours long instead of silently short
