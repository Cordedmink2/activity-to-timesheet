...
- **Break rows**: *only* for breaks ≥17.5 min (or the user's overridden threshold). Render `*Break*` in italic, `—` em-dash in the Client column. Shorter AFK gaps fold silently into the surrounding work block. A break a **corroborated calendar** block covers (`SKILL.md` Step 3) keeps its row — the AFK watcher recorded it and this file records what the instruments said — and the entry over it is declared in the Notes section, naming the event, the same way the table's declaration does. Deleting the row instead would leave the file agreeing with the blocks and disagreeing with the skeleton, which is the one thing the reader of an audit trail needs to be able to see.
