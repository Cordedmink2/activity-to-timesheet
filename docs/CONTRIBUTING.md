...
| No assumed timezone: with nothing configured a run reads the machine's own zone and labels it *derived from this machine* wherever the user reads it (#30); a run that can read neither refuses rather than guessing an offset | `scripts/timezone.py` `resolve_zone_with_source()` docstring, and `DERIVED` beside it for the word the label carries | `skills/daily/tests/test_declared_configuration.py` — the derived zone is used and labelled, a configured one wins, a machine zone that does not load gets the same refusal as none, and the unconfigured user's refusal names both ways to supply a zone, never an offset; `skills/daily/tests/test_references.py` holds the prose copies that carry the label to the constant's word | `SKILL.md` "Timezone" and the Step 8 confirmation's zone line, `references/output-format.md` §Conventions (the Notes line the label becomes), `references/activitywatch.md` §"Time zones", `references/first-run.md`'s first-run table, `.env.example`, `README.md`'s install table, the `reconcile` skill's configuration-gap paragraph, and the module docstring of every script that resolves a zone — `afk_blocks`, `activity_timeline`, `harvest_post` and, since #32, `harvest_patch` |
...
| The transition instant carries two clock readings — `03:00` as you reach it and `02:00` once it has passed — so an entry split there reads as an overlap and is not one | `scripts/harvest_write.py` `refusal_for_a_straddled_change()`, the copy a run is shown, which the create and the patch both call; why it sits with the write path rather than the zone arithmetic, and why the patch imports it rather than restating it, is `docs/skills/daily/decision-log.md` §"A create straddling the fall-back was billed an hour short, silently" | `test_timezone.py` (`transition_clocks()`) and `test_edge_harvest_api.py` (the refusal, the hour it saves, and the patch's message asserted equal to the create's) | `scripts/timezone.py` `transition_clocks()` docstring, `references/output-format.md` §Conventions (the cases the refusal cannot see), `SKILL.md` Step 9, `CHANGELOG.md`. A prose copy has already been wrong once — `docs/skills/daily/decision-log.md` §"Two instants an hour apart printed the same clock time" — so check the arithmetic sums to the elapsed time before changing any copy |
...
| Standing the screenshot task up: what `setup_screenshot_pipeline.ps1` registers — the schedule it fires on, the directory it captures into, the script it runs *in place* — and the two shells that clear an `Access is denied` for good, because re-running elevated recreates the same `BUILTIN\Administrators` owner | the `setup` skill's `SKILL.md` step 5 | the repo-level `tests/test_setup_skill.py` — the `-StartTime` and `-EndTime` the script's own `param()` block binds, held against the *paragraph that runs the script* rather than against the step, because the **Do** above it already names the same two in an aside and a whole-step check passes with the owning paragraph deleted; and the elevated-task fix counted across every `skills/**/*.md` and refused a second copy. Both are scoped to `skills/` and cannot see `README.md` | `README.md` § 10, the third copy this row was written for and the one #38 deletes with the rest of the walkthrough; `references/first-run.md` § "Screenshot capture", now a pointer here plus the health check it owns outright — a task that was working and stopped; the `reconcile` skill's capture-less-days finding, which names the health check alone. The split is #39's, and the line it draws is **standing one up against diagnosing one that has stopped** — the same line those two files already claim in their own headers |
| Getting a Harvest account ID and a personal access token, and what makes them reach a running session | `references/first-run.md` §"First-run: configuration", which is the copy carrying *why* — the precedence the seam applies, where a sensitive value actually sits on this platform, and why a new session is what delivers it | the repo-level `tests/test_plugin_config.py`, which counts the shipped files naming `id.getharvest.com` and refuses a second. It holds the count rather than the wording: the `setup` skill routes a missing credential to the configure dialog and must never collect one itself, which is the copy worth preventing | `README.md` § 8, now a pointer at the install table above it and at this owner, and `README.md`'s install table, which is the reading version of where the values are stored; `SKILL.md` Prerequisite 5, which checks the credentials work and routes a failure here. The install table keeps the `id.getharvest.com` address a reader needs before installing anything, so the guard stays scoped to `skills/` rather than widening to the whole repo |
...
| Work kind and task selection | `references/classification-rules.md` | Step 4 points at it |
| The disambiguation ladder and the interleaved-day switch-point protocol | `references/disambiguation.md` | Step 5, which is a pointer at it and nothing else; Step 4's interleaved-day line; `classification-rules.md` §5, §6 and its interleaved-day pointer. The split is #40's, and the line it draws is **what raises a flag against what resolves one**: `classification-rules.md` is read on every run, so everything that can *raise* a 🔸 stays there — the confidence rating, and §5 and §6's two flag-on-sight rules. `disambiguation.md` loads only once a flag exists and cannot be what creates one. § "Environment-connected tools are a rubric signal, not only a disambiguation step" in the decision log is the measured case: a plain `XrmToolBox` title never *looks* ambiguous, so a rule sitting behind the flag is never reached |
...
| Screenshots never settle active/idle — nor does the calendar | `references/disambiguation.md` §"What the ladder does not decide" | `SKILL.md` folder mechanics and Step 5's one-line version, Step 3's calendar paragraph ("the calendar re-infers idle no more than a screenshot does"), `classification-rules.md` §"Focused window ≠ active attention", `docs/adr/0008-…` §Context, which is the rule the whole calendar decision was shaped around |
| Check the other monitors before trusting one | `classification-rules.md` §"Focused window ≠ active attention" | `SKILL.md` folder mechanics, `references/disambiguation.md`'s subagent brief and its interleaved-days probe step, `classification-rules.md` long-agent-CLI and browser-row paragraphs |
| The published configuration reaches the **Bash** tool and nothing else, so every read of a configured value happens there and PowerShell is handed the resolved literal | `scripts/skill_config.py` `note_for_an_unreached_shell()`, the copy a run is shown; why the scope belongs to the harness and no test here can hold it — which is why this row is in this table — is `docs/skills/daily/decision-log.md` §"Two ways the configuration does not arrive, and only one had a name" | the precedence block at the top of the same module, `hooks/publish_plugin_config.py` module docstring and its `.sh` wrapper's second known limitation, `SKILL.md` "Running the scripts" + "Screenshot location", the `reconcile` skill's screenshot-index step, the `setup` skill's "Before you start" probe + step 5, `references/first-run.md` §"When the configuration does not arrive" cause 1, `README.md` §Updating step 2 and §8 step 3, the repo-level `tests/test_plugin_config.py`, `tests/test_setup_skill.py`'s two cross-skill guards, `skills/daily/tests/conftest.py`'s session-marker pin and `skills/daily/tests/test_config_seam.py`'s `REACHED`/`UNREACHED` constants (which encode whatever the check believes, so they move first when the check does), `scripts/calendar_day.py`'s off message and `skills/daily/tests/test_calendar_day.py`'s unreached-shell test, the decision log's §"The calendar toggle and the wrapper behind it", `CHANGELOG.md` |
...
| The ways a tag or a category rule fails silently: a bracketed rule against a bare tag (or the reverse), a pattern that is only the client's name, one broad enough to take the label off a correct rule, spaces inside an alternation, a tag left on a general profile | the `setup` skill's `SKILL.md` step 4 §"If it fails", which lists them in the order each is actually the answer and quotes the gate's own refusals | `demo/tag-rule-demo.html`, where each scenario is one of these run live — the executable copy, and faster to open than to explain; `README.md` §§"Tag the browser profiles…" and "The category rules", the reading copy — § 4 no longer enumerates the refusals, and points at the demo and the step instead; #38 has to find a home for what is left, along with the demo link. The row above owns the *procedure* these are the failures of; this row is the failure list alone. `scripts/category_rules.py` owns which refusals exist and the words each one uses, and `skills/daily/tests/test_category_rules.py` holds them there — what no instrument reaches is a prose list staying in step with that set, which is why this row is in this table and not the one above |
