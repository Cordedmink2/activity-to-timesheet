...
| Every flag a script parses is also named in `SKILL.md`'s "Files in this skill" entry for it — presence only; what the flag *does* is written once, wherever it is best explained | the script's own parser | `skills/daily/tests/test_references.py`, held against the parser | `SKILL.md` "Files in this skill". The `reconcile` skill's `SKILL.md` is a *third* copy (it invokes `harvest_list.py --by-day` and names `--window` and `--full`) and nothing holds it; rename a flag and it rots silently |
...
| The click-path through the URL-in-title options page and the activity source's Categories UI — where each lives, what to type, and that a category is saved per client rather than once at the end | the `setup` skill's `SKILL.md` steps 3 and 4, which are what a run reads out | `README.md` §"Set the title format" and §"Configure ActivityWatch categories", which are the reading version of the same two procedures. These drifted on save order before anyone noticed, and the README still names a nested `Work > Acme` where the skill keeps the category flat — the timeline joins a name path with `>`, so the two are not interchangeable |
